...
public class HomeDAO {
   ...
   static String week=" jdep >= DATE_FORMAT(NOW() - INTERVAL WEEKDAY(NOW()) DAY, '%Y-%m-%d 00:00:00') AND jdep <= LAST_DAY(CURRENT_DATE())";
    static String month=" jdep >= DATE_FORMAT(NOW(), '%Y-%m-01 00:00:00') AND jdep <= LAST_DAY(CURRENT_DATE())";


    public ObservableList<Vol> getall(String period) throws SQLException {

        Connection connection=Myconnection.connect();
        String sql = "SELECT * FROM vol WHERE "+period;
    ...
    public static int get_passengers_number(String period)  {
        String sql = "SELECT count(*) FROM vol v , reservation r  WHERE v.ID_vol=r.ID_vol and "+period;
        return(get_number(sql));
    }
